...
use App\Services\StudentSchooling;
...
class StudentController extends AbstractController
{
     ...
     */
    public function add(Request $request, EntityManagerInterface $em, StudentRepository $studentRepository): Response
    {
        $student = new Student();
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $em->persist($student);
            $em->flush();
            $this->addFlash("succesinscriptionetudiant", "Etudiant inscrit avec succès!");
            return $this->redirectToRoute('ajoutstudent');
        }

        return $this->render('student/index.html.twig', [
            'form' => $form->createView(),
            'listestudent' => $studentRepository->findBySomeLimit(5)
        ]);
    }
    ...
    /**
     * @Route("/etudiant/etat-scolarite", name="etatscolarite")
     *
     * @param StudentRepository $studentRepository
     * @return void
     */
    public function statusPaiementEtudiant(StudentSchooling $studentSchooling)
    {

        return $this->render('student/statutpaiements.html.twig', [
            "students" => $studentSchooling->paidSum()
        ]);
    }
    ...
    private function getData(): array
    {
        ...
        $student = $this->entityManager->getRepository(Student::class)->findAll();

        foreach ($student as $students) {
            $list[] = [
                $students->getMatricule(),
    ...
    /**
     * 
     *@Route("/etudiant/liste/export", name="exportdata")
     * 
     */
    public function expordata(StudentRepository $studentRepository)
    {
    ...
    /**
     * @Route("/pdf", name="exportpdf")
     *  
     * */
    public function pdfFile(StudentRepository $studentRepository)
    {
        $mpdf = new \Mpdf\Mpdf();
        $x = $studentRepository->findAll();
...
<body>
<div class="shadowtitle">New Factures en PDF</div>
<h3>HTML5 tags</h3>
...
' . implode("',  '", $x) . '<br>';
        ...
        $mpdf->SetHeader('Mypdf_file   {PAGENO}');
        $mpdf->WriteHTML($html);
        $mpdf->Output();
    }
}
